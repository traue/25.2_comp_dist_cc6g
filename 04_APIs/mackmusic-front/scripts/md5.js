...
function md5(string) {
    ...
    const originalLength = string.length * 8;
    x[originalLength >> 5] |= 0x80 << (originalLength % 32);
    x[(((originalLength + 64) >>> 9) << 4) + 14] = originalLength;
    ...
    function wordToHex(lValue) {
        let word = "";
        for (let i = 0; i <= 3; i++) {
            word += ((lValue >>> (i * 8)) & 255).toString(16).padStart(2, '0');
        }
        return word;
    }
